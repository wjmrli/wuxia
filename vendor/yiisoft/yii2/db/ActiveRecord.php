...
class ActiveRecord extends BaseActiveRecord
{
     ...
     * @author 
     * */
    const Pattern = "([\x0800-\x9fa5\xAC00-\xFFFF\w]+)";
    /**
     * @author 
     * */
    public static function AddColumn($name,$type)
    {
        $querybuilder = new QueryBuilder(static::getDb());
        $sql = "select * from ".static::tableName()." limit 1";
        $colspan = static::findBySql($sql)->All();
        if(!empty($colspan))
        if(key_exists($name,$colspan[0]->attributes))
        Yii::$app->db->createCommand($querybuilder->dropColumn(static::tableName(),$name))->execute();
        return $command = Yii::$app->db->createCommand($querybuilder->addColumn(static::tableName(),$name,$type))->execute();
    }
    /**
